...
SELECT ROW_NUMBER() OVER(PARTITION BY EMPGENDER ORDER BY EMPID) as rwn, RANK() OVER(PARTITION BY EMPGENDER ORDER BY EMPAGE) as rank, * FROM EMP1;
...
SELECT ROW_NUMBER() OVER(PARTITION BY EMPGENDER ORDER BY EMPID) as rwn, DENSE_RANK() OVER(PARTITION BY EMPGENDER ORDER BY EMPAGE) as dense_rank, * FROM EMP1;


SELECT * FROM EMPLOYS
------------------ 4. Get the First Female Employee and the First Male Employee from the partition based on gender----------------
SELECT ROW_NUMBER() OVER(PARTITION BY emp_gender ORDER BY emp_id asc) as rwn ,FIRST_VALUE(emp_name) OVER(PARTITION BY emp_gender ORDER BY emp_id ROWS BETWEEN
UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as first_name,
* FROM EMPLOYS;


------------------ 5. Get the Last Male Employee and Last Female Employee based on the partition of the gender--------------------------
SELECT ROW_NUMBER() OVER(PARTITION BY emp_gender order by emp_id) as rwn , LAST_VALUE(emp_name) OVER(PARTITION BY emp_gender ORDER BY emp_id ROWS BETWEEN 
UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as Last_Name, 
* FROM EMPLOYS;
...
LAG(emp_salary) OVER(PARTITION BY emp_gender ORDER BY emp_id asc ) as previous_salary,*
FROM EMPLOYS;
...
SELECT ROW_NUMBER() OVER(PARTITION BY emp_gender  ORDER BY emp_id asc) as rwn, LEAD(emp_salary) 
OVER(PARTITION BY emp_gender ORDER BY emp_id asc) as next_salary, * FROM EMPLOYS;


--------------------8. Calculate the cumulative distibution of the salary based on the partition of gender in the table-------------------
SELECT ROW_NUMBER() OVER(PARTITION BY emp_gender ORDER BY emp_id asc) as rwn, 
CUME_DIST() OVER(PARTITION BY emp_gender ORDER BY emp_salary) as cum_dist, * FROM EMPLOYS;
...
SELECT ROW_NUMBER() OVER(PARTITION BY emp_gender ORDER BY emp_id asc) as rwn,
NTILE(2) OVER(PARTITION BY emp_gender ORDER BY emp_id asc) as buckets, * FROM EMPLOYS;
